...
{{- else }}
    ["{{ $w.Name }}", "{{ $w.MagType }}", "{{ $w.RailType }}", "{{ $w.Camoflage }}", "{{ $w.AllowsMuzzleDevices }}", "{{ $w.Features.AllowsGripsBipods }}", "{{ $w.Features.HasBipod }}", "{{ $w.Features.HasFrontGrip }}"],     {{if eq $w.RawScore 0.0}}{{ $w.Score }}{{else}}{{ $w.RawScore }}{{end}}{{if IsLastItem $i (len $w_array)}}{{else}},{{end}}
{{- end }}
{{- end }}
];
...
{{- if eq $w.IsStarting 1 }}
    {{- if $printed_once }}, {{end}}["{{ $w.Name }}", "{{ $w.MagType }}", "{{ $w.RailType }}", "{{ $w.Camoflage }}", "{{ $w.AllowsMuzzleDevices }}", "{{ $w.Features.AllowsGripsBipods }}", "{{ $w.Features.HasBipod }}", "{{ $w.Features.HasFrontGrip }}"]{{ $printed_once = true}}
{{- end }}
{{- end }}
{{- end }}
];
