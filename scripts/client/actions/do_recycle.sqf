...
if (not (typeOf _vehtorecycle in [ammobox_b_typename, ammobox_o_typename, ammocrate_o_typename])) then {
...
if (dorecycle == 1 && !(isnull _vehtorecycle) && alive _vehtorecycle) then {
    private _price = [round ((_objectinfo select 2) * GRLIB_recycling_percentage), 0] select (docollect != 0);
    [_vehtorecycle,  _price] remoteExecCall ["recycle_remote_call", 2];
};
