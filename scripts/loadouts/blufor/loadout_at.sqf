...
private ["_mainWeapon", "_mainMags1", "_mainMags2", "_optics", "_opticsCompat", "_launcher", "_atMags", "_weaponCandidate", "_tmpArray"];
...
_tmpArray = GRLIB_whitelisted_from_arsenal - GRLIB_arsenal_aaSystems - GRLIB_arsenal_pistols - GRLIB_arsenal_machineguns - GRLIB_arsenal_autoRifles - GRLIB_arsenal_sniperRifles - GRLIB_arsenal_marksmanRifles - GRLIB_arsenal_carbines - GRLIB_arsenal_rifles - GRLIB_arsenal_specWeapons - GRLIB_arsenal_launchers;

while {_launcher == ""} do {
	_weaponCandidate = selectRandom GRLIB_arsenal_atSystems;
	diag_log format ["LOADOUTS: AT candidate: %1", _weaponCandidate];
	if (_weaponCandidate in _tmpArray) then {_launcher = _weaponCandidate};
	sleep 0.1;
};
diag_log format ["LOADOUTS: AT winner: %1", _launcher];
...
_atMags = selectRandom (getArray (configFile / "CfgWeapons" / _launcher / "magazines"));

diag_log format ["LOADOUTS: magazines: %1, %2, %3", _mainMags1, _mainMags2, _atMags];
...
for "_i" from 1 to 2 do {_unit addItemToVest _mainMags1;};
for "_i" from 1 to 2 do {_unit addItemToVest _mainMags2;};
_unit addBackpack "B_Carryall_oli";
for "_i" from 1 to 3 do {_unit addItemToBackpack _atMags;};
