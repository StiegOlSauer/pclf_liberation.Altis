...
private ["_mainWeapon", "_mainMags1", "_optics", "_opticsCompat", "_weaponCandidate", "_tmpArray", "_pistol", "_pistolMags"];
...
_pistol = "";
_optics = "";
_tmpArray = GRLIB_whitelisted_from_arsenal - GRLIB_arsenal_aaSystems - GRLIB_arsenal_pistols - GRLIB_arsenal_launchers - GRLIB_arsenal_machineguns - GRLIB_arsenal_autoRifles - GRLIB_arsenal_marksmanRifles - GRLIB_arsenal_carbines - GRLIB_arsenal_rifles - GRLIB_arsenal_specWeapons - GRLIB_arsenal_atSystems;

//diag_log format ["LOADOUTS: _tmpArray for main gun: %1", _tmpArray];
while {_mainWeapon == ""} do {
	_weaponCandidate = selectRandom GRLIB_arsenal_sniperRifles;
...
if (_opticsCompat == "CUP_optic_SB_3_12x50_PMII") then {_optics = selectRandom GRLIB_arsenal_opticsSniperNATO};
if (_opticsCompat == "HLC_Optic_PSO1") then {_optics = selectRandom GRLIB_arsenal_opticsSniperAK};
...
if (_mainWeapon in ["CUP_srifle_AS50","CUP_srifle_CZ750","CUP_srifle_LeeEnfield_rail"]) then {_optics = selectRandom GRLIB_arsenal_opticsSniperNATO};
if (_mainWeapon == "CUP_srifle_ksvk") then {_optics = "CUP_optic_PSO_3"};
...
diag_log format ["LOADOUTS: magazines: %1, %2", _mainMags1, _pistolMags];
...
for "_i" from 1 to 7 do {_unit addItemToVest _mainMags1;};
