...
while { true } do {
	...
	private _running_scripts = diag_activeScripts;
	...
	_myfpsmarker setMarkerText format [ "%1: %2 fps, %5 spawned scripts, %6 execVM scripts, %3 units, %4 vehicles", _sourcestr, ( round ( _myfps * 100.0 ) ) / 100.0 , _localunits, _localvehicles , _running_scripts select 0, _running_scripts select 1];
