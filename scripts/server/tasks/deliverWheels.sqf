...
private ["_taskPrefix", "_taskMarker", "_taskCity","_base_objects", "_unitToSpawn", "_deliveredWheels", "_arrayOfPositions", "_ammoboxes", "_taskStates", "_pos", "_spawnedAlready", "_prepareInProgress"];

_taskPrefix = "FREEDOMWHEELS";
_taskStates = [];
_base_objects = [];
_deliveredWheels = [];
_arrayOfPositions = [];
...
while {(count _deliveredWheels < 4) || (_prepareInProgress)} do {
	...
	sleep 2;
	_deliveredWheels = nearestObjects [(getMarkerPos _taskMarker), ["Man"], 30];
	...
	if ((([_taskMarker] call BIS_fnc_taskState) == "ASSIGNED") && !(_spawnedAlready)) then {
		...
		_truck setHit ["wheel_1_1_steering", 1];
		_truck setHit ["wheel_2_2_steering", 1];
		_truck lock 2;
