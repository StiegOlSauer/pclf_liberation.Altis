...
"_airfield", "_dudes", "_dudesGroup", "_dudeMobile", "_escapePlane", "_escapePlaneType", "_distanceOk", "_crewOk", "_duke", "_mike", "_debug"];
...
while {(({ alive _x } count _dudes) > 0) && !(_crewOk) || ((_phaseOnePrepare) && !(_phaseOneDone))} do {
	...
	if (((count _dudes) > 0) && (alive _escapePlane)) then {		
		...
		if ((alive _duke) && (alive _mike) && ((_duke in (crew _escapePlane))) && ((_mike in (crew _escapePlane)))) then {
			_crewOk = true;
		};
		
		if ((alive _duke) && !(alive _mike) && ((_duke in (crew _escapePlane)))) then {
			_crewOk = true;
		};		
		
		if (!(alive _duke) && (alive _mike) && ((_mike in (crew _escapePlane)))) then {
			_crewOk = true;
		};		
	};
...
while {((({ alive _x } count _dudes) > 0) && (alive _escapePlane) && !(_distanceOk)) || ((_phaseOneDone) && (_phaseTwoPrepare))} do {	
	...
	if (((count _dudes) > 0) && (alive _escapePlane)) then {		
		...
		if ((((alive _duke) && (_duke in (crew _escapePlane))) || ((alive _mike) && (_mike in (crew _escapePlane)))) && (({_escapePlane distance (getMarkerPos _x) < 8000} count sectors_allSectors) == 0)) then {			
			_distanceOk = true;
		};
	};
	...
	if ((([_taskMarker] call BIS_fnc_taskState) == "ASSIGNED") && !(_phaseTwoSpawned)) then {
		...
		{[_x] joinSilent _dudesGroup;} foreach _dudes;
		...
		0 = [_taskMarker, ["You need to cross Altis border and get your passengers to nearest friendly democracy. Fly away from any sector for at least 8 km.", "Fly away from Altis", "Running men"]] call BIS_fnc_taskSetDescription;
...
if ((count (crew _escapePlane) > 0) && (alive _escapePlane)) then {_base_objects = _base_objects - [_escapePlane]};
if ((count (crew _dudeMobile) > 0) && (alive _escapePlane)) then {_base_objects = _base_objects - [_dudeMobile]};
{sleep 0.1; deleteVehicle _x;} foreach _base_objects;
deleteMarker _debug;